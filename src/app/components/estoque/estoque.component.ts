...
export class EstoqueComponent implements OnInit {
    ...
    // Chaves i18n dos motivos de movimentação (o texto em si é definido em
    // MOTIVOS_MOVIMENTACAO no model, mas isso é a base pt-BR mockada — pra
    // exibir/traduzir na tela usamos essas chaves em vez do texto fixo.
    readonly MOTIVOS_KEYS: Record<MotivoMovimentacao, string> = {
        venda: 'estoque.motivo.venda',
        cancelamento: 'estoque.motivo.cancelamento',
        devolucao: 'estoque.motivo.devolucao',
        compra: 'estoque.motivo.compra',
        ajuste_manual: 'estoque.motivo.ajuste_manual',
        inventario: 'estoque.motivo.inventario',
    };

    // Opções { label, value } pros p-dropdown (PrimeNG) desta tela.
    readonly STATUS_ESTOQUE_OPTIONS = [
        { label: 'estoque.filters.status_all', value: 'todos' },
        { label: 'estoque.status.ok', value: 'ok' },
        { label: 'estoque.status.low', value: 'baixo' },
        { label: 'estoque.status.out', value: 'zerado' },
    ];
    readonly TIPO_MOV_OPTIONS = [
        { label: 'estoque.filters.type_all', value: 'todos' },
        { label: 'estoque.mov_type.in_plural', value: 'entrada' },
        { label: 'estoque.mov_type.out_plural', value: 'saida' },
        { label: 'estoque.mov_type.adjustment_plural', value: 'ajuste' },
    ];
    readonly TIPO_AJUSTE_OPTIONS = [
        { label: 'estoque.adjust.type_in', value: 'entrada' },
        { label: 'estoque.adjust.type_out', value: 'saida' },
    ];
    ...
    readonly motivoOptions: { label: string; value: MotivoMovimentacao }[] =
        Object.entries(MOTIVOS_MOVIMENTACAO).map(([value]) => ({ value: value as MotivoMovimentacao, label: this.MOTIVOS_KEYS[value as MotivoMovimentacao] }));
    ...
    statusLabel(status: StatusEstoque): string {
        return { ok: 'estoque.status.ok', baixo: 'estoque.status.low', zerado: 'estoque.status.out' }[status];
    }
    ...
    tipoMovLabel(tipo: TipoMovimentacao): string {
        return { entrada: 'estoque.mov_type.in', saida: 'estoque.mov_type.out', ajuste: 'estoque.mov_type.adjustment' }[tipo];
    }
